...
function Pasajero({ usuario, onLogout, onUpdateUser }) {
	...
	useEffect(() => {
		...
		const map = L.map(mapRef.current, {
			center: [11.4145, -69.65], // Centrado conveniente en el corredor Coro-Calderas
			zoom: 13,
			minZoom: 12,
			maxZoom: 17,
			maxBounds: [
		...
		try {
			const resp = await fetch(`${API_BASE_URL}/api/incidencias`, {
				...
				body: JSON.stringify({
					...
					autor: nombrePasajero,
					rol: "Pasajero",
				}),
			});
			if (resp.ok) {
				alert(
					"¡Reporte enviado! El administrador lo revisará pronto. Gracias por ayudarnos a mejorar.",
				);
				setIncidencia("");
			} else {
	...
	const handleGuardarSeguridad = async (e) => {
		...
		try {
			...
			if (resp.ok) {
				alert(
					"Seguridad y pregunta de recuperación actualizadas exitosamente.",
				);
		...
		<div className="dashboard-container">
			...
			<aside className={`sidebar${menuAbierto ? " sidebar-open" : ""}`}>
				...
				<nav className="sidebar-menu">
					<button
						className={getSidebarClass("mapa")}
						onClick={() => {
							setTab("mapa");
					...
					<button
						className={getSidebarClass("horarios")}
						onClick={() => {
							setTab("horarios");
							setParadaSeleccionada(null);
							cerrarMenu();
			...
			<main className="main-content">
				...
				{tab === "mapa" && (
					...
					>
						<p className="slogan" style={{ margin: 0 }}>
							Selecciona una parada en el mapa para ver los últimos arribos en
							tiempo real. El mapa está limitado al área metropolitana de Coro y
							Las Calderas.
						</p>
						...
						<div style={{ display: "flex", gap: "25px", flexWrap: "wrap" }}>
							{/* Contenedor del Mapa */}
							<div
								style={{ flex: "2", minWidth: "350px", position: "relative" }}
							>
								...
								{!paradaSeleccionada ? (
									...
									>
										...
										<p style={{ fontSize: "0.9rem", margin: 0 }}>
											Haz clic sobre cualquiera de las paradas en el mapa para
											desplegar los tiempos y buses recientes.
										</p>
									</div>
								) : (
									<div
									...
									>
										{llegadasFiltradas.length === 0 ? (
											...
											>
												No se han registrado llegadas de buses el día de hoy en
												esta estación.
											</p>
										) : (
											llegadasFiltradas.slice(0, 5).map((ll, idx) => (
												<div
													...
													style={{
														background:
															"linear-gradient(135deg, rgba(59, 130, 246,0.12), rgba(39,174,96,0.02))",
														padding: "12px 15px",
														borderRadius: "10px",
				...
				{tab === "horarios" && (
					<div style={{ maxWidth: "800px" }}>
						<p className="slogan">
							Listado general histórico con todos los arribos registrados por
							los fiscales en toda la ruta.
						</p>
								...
								{listaLlegadas.map((ll, idx) => (
									...
									>
										...
										<div style={{ textAlign: "right" }}>
											...
											>
												{ll.fecha
													? new Date(ll.fecha).toLocaleDateString()
													: "Hoy"}
											</small>
										</div>
									</div>
				...
				{tab === "novedad" && (
					...
					>
						...
						>
							¿Has notado retrasos excesivos, conductas inapropiadas o problemas
							con las unidades? Envíanos tu reporte. Será procesado por la
							administración para mejorar el servicio.
						</p>
						<form
