...
function Admin({ usuario, onLogout, onUpdateUser }) {
	...
	const fetchHomeStats = async () => {
		...
		try {
			...
			const personalConRol = usuarios.filter(
				(u) => u.rol === "chofer" || u.rol === "fiscal",
			);
			...
			const personalTurno =
				unidades.filter((u) => u.choferAsignado && unidadesEnRuta.has(u.nombre))
					.length || Math.min(unidadesActivas, personalConRol.length);

			const incsPendientes = incidencias.filter(
				(i) => !i.resuelta && !i.cerrada,
			);
			...
			const eventos = [
				...
				...incidencias.slice(-10).map((inc) => ({
					hora: inc.createdAt
						? new Date(inc.createdAt).toLocaleTimeString("es-VE", {
								hour: "2-digit",
								minute: "2-digit",
							})
						: "--:--",
			...
			setHomeStats({
				...
				incidenciasNombres: incsPendientes
					.slice(0, 3)
					.map(
						(i, idx) =>
							`#${idx + 23} ${i.descripcion ? i.descripcion.slice(0, 18) : "Novedad"}...`,
					),
			});
	...
	const getSidebarClass = (current) =>
		tab === current ? "sidebar-btn active" : "sidebar-btn";
	...
	const operativas = listaUnidades.filter(
		(u) => u.estado === "Operativa",
	).length;
	const enTaller = listaUnidades.filter(
		(u) => u.estado === "Mantenimiento" || u.estado === "En taller",
	).length;
	const inactivas = listaUnidades.filter(
		(u) => u.estado === "Inhabilitada" || u.estado === "Inactiva",
	).length;

	const totalChoferes = choferesArray.length;
	const choferesAsignadosNombres = new Set(
		listaUnidades.map((u) => u.choferAsignado).filter(Boolean),
	);
	...
	const criticasCount = incidenciasFiltradas.filter(
		(inc) => inc.severidad === "Crítica",
	).length;
	const moderadasCount = incidenciasFiltradas.filter(
		(inc) => inc.severidad === "Moderada" || !inc.severidad,
	).length;
		...
		<div className="dashboard-container">
			...
			<aside className={`sidebar${menuAbierto ? " sidebar-open" : ""}`}>
				...
				<nav className="sidebar-menu">
					...
					<button
						className={getSidebarClass("registro")}
						onClick={() => {
							setTab("registro");
							cerrarMenu();
						}}
					...
					<button
						className={getSidebarClass("usuarios")}
						onClick={() => {
							setTab("usuarios");
							cerrarMenu();
						}}
					...
					<button
						className={getSidebarClass("unidades")}
						onClick={() => {
							setTab("unidades");
							cerrarMenu();
						}}
					...
					<button
						className={getSidebarClass("incidencias")}
						onClick={() => {
							setTab("incidencias");
							cerrarMenu();
						}}
					...
					<button
						...
						onClick={() => {
							setTab("paradas");
							setParadaAdminSeleccionada(null);
							cerrarMenu();
						}}
					>
			...
			<nav className="bottom-nav">
				...
				<button
					className={`bottom-nav-item ${tab === "usuarios" ? "active" : ""}`}
				...
				<button
					className={`bottom-nav-item ${tab === "unidades" ? "active" : ""}`}
				...
				<button
					className={`bottom-nav-item ${tab === "incidencias" ? "active" : ""}`}
				...
				<button
					className={`bottom-nav-item ${tab === "reporte" ? "active" : ""}`}
			...
			<main className="main-content">
					...
					<div className="fade-in-tab">
						...
						<div
							style={{
								...
								border: `1px solid var(--neon-header-border)`,
								borderRadius: "16px",
								padding: "24px 30px",
								marginBottom: "32px",
						...
						>
							...
							>
								<span style={{ fontWeight: 400 }}>HOLA, </span>
								<span style={{ fontWeight: 800 }}>ADMINISTRADOR PRINCIPAL</span>
							</h1>
							<button
								onClick={fetchHomeStats}
								disabled={loadingHome}
								style={{
									background: "var(--neon-icon-bg)",
									border: "none",
							...
							>
								...
								<div
									...
									onMouseEnter={(e) => {
										...
										e.currentTarget.style.boxShadow =
											"0 12px 40px rgba(0,0,0,0.15), var(--neon-shadow)";
									}}
									onMouseLeave={(e) => {
										e.currentTarget.style.transform = "translateY(0)";
								...
								>
									...
									<div
										style={{
											marginTop: "16px",
											height: "4px",
											background: "var(--neon-progress-track)",
											borderRadius: "2px",
											overflow: "hidden",
										}}
									>
										<div
											style={{
												height: "100%",
												width: `${homeStats.unidadesTotal > 0 ? (homeStats.unidadesActivas / homeStats.unidadesTotal) * 100 : 0}%`,
												background: "var(--neon-icon1-color)",
												borderRadius: "2px",
												boxShadow: "0 0 10px var(--neon-icon1-color)",
								...
								>
									...
									>
										<div
											style={{
												height: "100%",
												width: `${homeStats.personalTotal > 0 ? (homeStats.personalTurno / homeStats.personalTotal) * 100 : 0}%`,
												background: "var(--neon-icon2-color)",
												borderRadius: "2px",
												boxShadow: "0 0 10px var(--neon-icon2-color)",
												transition: "width 0.8s ease, background 0.3s",
											}}
										/>
									</div>
								...
								<div
									...
									onMouseEnter={(e) => {
										e.currentTarget.style.transform = "translateY(-4px)";
										e.currentTarget.style.boxShadow =
											"0 12px 40px rgba(0,0,0,0.15), var(--neon-card3-shadow)";
									}}
									onMouseLeave={(e) => {
										e.currentTarget.style.transform = "translateY(0)";
										e.currentTarget.style.boxShadow =
											"var(--neon-card3-shadow)";
									}}
								>
									<div
										style={{
									...
									>
										{homeStats.incidenciasNombres &&
										homeStats.incidenciasNombres.length > 0
											? homeStats.incidenciasNombres.join(", ")
									...
									>
										<div
											style={{
												height: "100%",
												width:
													homeStats.incidenciasPendientes > 0 ? "100%" : "0%",
												background: "var(--neon-icon3-color)",
												borderRadius: "2px",
												boxShadow: "0 0 10px var(--neon-icon3-color)",
												transition: "width 0.8s ease, background 0.3s",
											}}
										/>
									</div>
										...
										>
											...
											<div
												style={{
													...
													flexShrink: 0,
													background:
														ev.tipo === "incidencia" ? "#e74c3c" : "#3b82f6",
													boxShadow:
														ev.tipo === "incidencia"
				...
				{tab === "registro" && (
					...
					>
						<div style={{ textAlign: "center", marginBottom: "30px" }}>
							...
							>
								Completa los datos detallados del personal de Ruta Express
							</p>
						</div>
				...
				{tab === "usuarios" && (
					...
					>
						...
						>
							Directorio y Expedientes Organizados
						</h2>
								...
								style={{
									...
									background:
										filtroRol === "chofer"
											...
											: "var(--card-bg)",
									color:
										filtroRol === "chofer" ? "#3b82f6" : "var(--text-muted)",
									fontWeight: 700,
									cursor: "pointer",
									display: "flex",
									alignItems: "center",
								...
								style={{
									...
									background:
										filtroRol === "fiscal"
											...
											: "var(--card-bg)",
									color:
										filtroRol === "fiscal" ? "#3498db" : "var(--text-muted)",
									fontWeight: 700,
									cursor: "pointer",
									display: "flex",
									alignItems: "center",
								...
								style={{
									...
									background:
										filtroRol === "admin"
											...
											: "var(--card-bg)",
									color:
										filtroRol === "admin" ? "#f39c12" : "var(--text-muted)",
									fontWeight: 700,
									cursor: "pointer",
									display: "flex",
									alignItems: "center",
						...
						{usuariosFiltrados.length === 0 ? (
							...
							>
								No hay coincidencias en los expedientes para este rol.
							</div>
						) : (
							<div
								style={{
						...
						<div style={{ display: "flex", gap: "20px", marginBottom: "30px" }}>
							...
							<div
								style={{
									flex: 1,
									background:
										"linear-gradient(to right, rgba(241,196,15,0.15), rgba(241,196,15,0.05))",
								...
								{incidenciasFiltradas.map((inc, index) => {
									...
									return (
										<div
											...
											style={{
												background: bgColor,
												border: `1px solid rgba(${esCritica ? "231,76,60" : "241,196,15"}, 0.3)`,
												borderRadius: "16px",
												padding: "20px",
												display: "flex",
												gap: "20px",
										...
										>
											...
											<div style={{ flex: 1 }}>
												...
												>
													...
													>
														{inc.autor
															? inc.autor.charAt(0).toUpperCase()
															: "?"}
													</div>
													<span
														style={{
				...
				{tab === "paradas" && (
					<div>
						{!paradaAdminSeleccionada ? (
							<div
								className="parada-list-container fade-in-tab"
								style={{ maxWidth: "1200px", margin: "0 auto" }}
							>
								<h2 style={{ color: "var(--text-main)", marginBottom: "20px" }}>
									Control Oficial de Paradas
								</h2>
								<div
									style={{
										display: "grid",
								...
								>
									...
									>
										...
										>
											...
											<button
												onClick={() => setParadaAdminSeleccionada("CALDERAS")}
												style={{
													background: "var(--card-border)",
													border: "none",
													color: "var(--text-main)",
										...
										>
											...
											{listaLlegadas.filter((ll) => ll.parada === "CALDERAS")
												.length === 0 && (
												<span
													style={{
														color: "var(--text-muted)",
									...
									>
										...
										>
											...
											<button
												onClick={() =>
													setParadaAdminSeleccionada("SAN ANTONIO")
												}
												style={{
													background: "var(--card-border)",
													border: "none",
										...
										>
											{listaLlegadas
												.filter((ll) => ll.parada === "SAN ANTONIO")
												.slice(0, 8)
												.map((ll, idx) => (
													<div
														key={idx}
													...
													>
														...
														<span
															style={{ color: "#f1c40f", fontSize: "0.75rem" }}
														>
															Active - OK
														</span>
													</div>
												))}
											{listaLlegadas.filter((ll) => ll.parada === "SAN ANTONIO")
												.length === 0 && (
												<span
													style={{
														color: "var(--text-muted)",
										...
										<button
											onClick={() => setParadaAdminSeleccionada("SAN ANTONIO")}
											style={{
												width: "100%",
												padding: "12px",
												marginTop: "20px",
									...
									>
										...
										>
											<div style={{ display: "flex", gap: "10px" }}>
												<div style={{ color: "#3498db", fontSize: "1.2rem" }}>
													📍
												</div>
												<div>
													<h3
											...
											<button
												onClick={() =>
													setParadaAdminSeleccionada("JUAN CRISOSTOMO FALCON")
												}
												style={{
													background: "var(--card-border)",
													border: "none",
										...
										>
											{listaLlegadas
												.filter((ll) => ll.parada === "JUAN CRISOSTOMO FALCON")
												.slice(0, 8)
												.map((ll, idx) => (
													<div
														key={idx}
													...
													>
														...
														<span
															style={{ color: "#3498db", fontSize: "0.75rem" }}
														>
															Active - OK
														</span>
													</div>
											...
											{listaLlegadas.filter(
												(ll) => ll.parada === "JUAN CRISOSTOMO FALCON",
											).length === 0 && (
												<span
													style={{
														color: "var(--text-muted)",
							...
							<div className="parada-list-container">
								...
								>
									<h2 style={{ color: "var(--text-main)", margin: 0 }}>
										Llegadas: {paradaAdminSeleccionada}
									</h2>
									<button
										className="btn-login"
										style={{
										...
										onClick={() => setParadaAdminSeleccionada(null)}
									>
										⬅ Volver a Paradas
									</button>
								</div>
								...
								<div style={{ display: "flex", gap: "40px" }}>
									<div style={{ flex: "1.5", minWidth: "350px" }}>
										{listaLlegadas.filter(
											(ll) => ll.parada === paradaAdminSeleccionada,
										).length === 0 ? (
											<p>No hay llegadas en esta parada.</p>
										) : (
											<div
												style={{
													display: "flex",
											...
											>
												{listaLlegadas
													.filter((ll) => ll.parada === paradaAdminSeleccionada)
													.map((ll) => (
														<div
															key={ll._id}
															style={{
									...
									{editandoId && (
										<div style={{ flex: "1", maxWidth: "350px" }}>
											...
											>
												Modificar Llegada Administrador
											</h3>
											<form onSubmit={handleModificarLlegada}>
												<div className="input-group">
													...
													<select
														value={paradaEdit}
														onChange={(e) => setParadaEdit(e.target.value)}
														style={{
															width: "100%",
															padding: "12px",
															background: "var(--card-bg)",
													...
													>
														...
														<option value="JUAN CRISOSTOMO FALCON">
				...
				{tab === "reporte" && (
					...
					>
						...
						>
							{[
								{
									label: "Total Llegadas Hoy",
									value: listaLlegadas.length,
									bg: "rgba(239, 68, 68, 0.15)",
									icon: "🚌",
								},
								{
									label: "Unidades Registradas Act.",
									value: listaUnidades.length,
							...
							<button
								onClick={async () => {
									...
									try {
										...
										doc.text("RUTA EXPRESS — REPORTE DIARIO (ADMIN)", 105, 16, {
											align: "center",
										});
										...
										doc.text(
											`Administrador: ${usuario.nombre || "Admin"}`,
											105,
											26,
											{ align: "center" },
										);
										doc.text(
											`Fecha: ${fechaStr}  |  Hora generación: ${horaStr}`,
											105,
											34,
											{ align: "center" },
										);
										...
										doc.setFontSize(13);
										doc.setFont("helvetica", "bold");
										doc.text("1. FLOTA REGISTRADA", 14, y);
										y += 5;
										autoTable(doc, {
											startY: y,
											head: [
										...
										if (llegadasFresh.length > 0) {
											...
											doc.setFontSize(13);
											doc.setFont("helvetica", "bold");
											doc.text("2. RESUMEN OPERACIONAL DEL DÍA", 14, y);
											y += 5;
											autoTable(doc, {
												startY: y,
												...
												body: [
													...
													[
														...
														horasOrd[0]?.horaLlegada || "N/A",
													],
													[
														"Hora cierre operaciones",
														horasOrd[horasOrd.length - 1]?.horaLlegada || "N/A",
													],
													[
														"Unidades operativas",
														unidadesFresh.filter(
															(u) => u.estado === "Operativa",
														).length,
													],
										...
										paradas.forEach((parada) => {
											const llParada = llegadasFresh.filter(
												(ll) => ll.parada === parada,
											);
											...
											autoTable(doc, {
												startY: y,
												head: [
													[`📍 ${parada} — ${llParada.length} llegada(s)`],
												],
												body: [],
												theme: "plain",
												headStyles: {
											...
											autoTable(doc, {
												startY: y,
												head: [["#", "Unidad", "Hora", "Registrado por"]],
												body: llParada.map((ll, i) => [
													i + 1,
													ll.nombreUnidad || "N/A",
													ll.horaLlegada || "N/A",
													ll.registradoPor || "N/A",
												]),
												theme: "grid",
												headStyles: {
													fillColor: [40, 120, 65],
										...
										if (todasLasVueltas.length > 0) {
											...
											doc.setFontSize(13);
											doc.setFont("helvetica", "bold");
											doc.text("4. CICLOS DE VUELTAS COMPLETAS", 14, y);
											y += 6;
											autoTable(doc, {
												startY: y,
												head: [
										...
										doc.save(
											`ReporteAdmin_${hoy.toISOString().split("T")[0]}.pdf`,
										);
									} catch (err) {
										console.error(err);
											...
											if (listVueltas.length === 0) {
												return (
													<tr>
														...
														>
															Ninguna unidad ha completado la secuencia de las 3
															paradas hoy todavía.
														</td>
													</tr>
												);
							...
							>
								...
								>
									...
									>
										...
										>
											{(usuario.nombre || usuario.usuario || "A")
												.charAt(0)
												.toUpperCase()}
										</div>
										<span
									...
									>
										...
										<div>
											...
											<input
												...
												placeholder="Ej: Toby / Caracas / Pérez..."
												required
												style={{
													width: "100%",
													padding: "14px 16px",
										...
										<div style={{ display: "flex", gap: "10px" }}>
											<button
												...
												style={{
													...
													padding: "14px",
													background:
														"linear-gradient(135deg, #3b82f6, #1abc9c)",
													border: "none",
													color: "var(--text-main)",
													borderRadius: "12px",
													fontWeight: 700,
				...
				>
					...
					>
						...
						>
							...
							>
								<p style={{ margin: 0 }}>
									<strong>👤 Usuario:</strong> {usuarioSeleccionado.usuario}
								</p>
								<p style={{ margin: 0 }}>
									<strong>🪪 Cédula:</strong>{" "}
									{usuarioSeleccionado.cedula || "N/A"}
								</p>
								<p style={{ margin: 0 }}>
									<strong>📧 Correo:</strong>{" "}
									{usuarioSeleccionado.correo || "N/A"}
								</p>
								<p style={{ margin: 0 }}>
									<strong>📞 Teléfono:</strong>{" "}
									{usuarioSeleccionado.telefono || "N/A"}
								</p>
								<p style={{ margin: 0 }}>
									<strong>📍 Dirección:</strong>{" "}
									{usuarioSeleccionado.direccion || "N/A"}
								</p>
								<p style={{ margin: 0 }}>
									<strong>⚙️ Estado Perfil:</strong>{" "}
									<span
										style={{
											color: usuarioSeleccionado.perfilCompletado
												? "#3b82f6"
												: "#e74c3c",
											fontWeight: 600,
										}}
									>
										{usuarioSeleccionado.perfilCompletado
											? "Completado"
			...
			{mostrarFormUnidad && (
				...
				>
					...
					>
						<button
							onClick={() => setMostrarFormUnidad(false)}
							style={{
								position: "absolute",
								top: "20px",
								right: "20px",
