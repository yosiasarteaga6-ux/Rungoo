...
function Login({ onLogin, onRecuperar }) {
  ...
  useEffect(() => {
    ...
    const map = L.map(publicMapRef.current, {
      ...
      minZoom: 12,
      maxZoom: 17,
      maxBounds: [[11.3800, -69.7200], [11.4600, -69.5800]],
      ...
      <div className="login-form-panel">
        ...
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '18px' }}>
          <div style={{
            ...
            borderRadius: '50%',
            background: 'linear-gradient(135deg, rgba(59, 130, 246,0.18), rgba(39,174,96,0.06))',
        ...
        <div style={{ textAlign: 'center', marginBottom: '25px' }}>
          ...
          <h1 className="brand-name">RUTA <span>EXPRESS</span></h1>
          <p className="login-calderas">✦ Transporte Colectivo Las Calderas ✦</p>
        </div>
        ...
        <form onSubmit={handleIngresar}>
          ...
          <button type="submit" className="btn-login" style={{ marginBottom: '12px' }}>
            🚀 INICIAR RUTA
          </button>
        </form>
        ...
        <button
          onClick={() => { setVerMapaPublico(true); setParadaSeleccionada(null); }}
          className="btn-login"
          style={{
            background: 'linear-gradient(135deg, rgba(52, 152, 219, 0.2), rgba(41, 128, 185, 0.05))',
        ...
        <div style={{ textAlign: 'center' }}>
          <button
            onClick={onRecuperar}
            style={{ background: 'none', border: 'none', color: 'rgba(59, 130, 246,0.7)', cursor: 'pointer', fontSize: '0.85rem', textDecoration: 'underline' }}
          >
            🔑 ¿Olvidaste tu contraseña?
          </button>
        </div>

        <p style={{ textAlign: 'center', marginTop: '20px', color: 'var(--text-muted)', fontSize: '0.7rem', letterSpacing: '1px' }}>
        ...
        <div className="public-map-modal">
          <div className="public-map-modal-content">
            <div className="public-map-modal-header">
              ...
              <button
                onClick={() => { setVerMapaPublico(false); setParadaSeleccionada(null); }}
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', margin: '0 0 20px 0', lineHeight: '1.4' }}>
              Consulta las estaciones y el historial de arribos en tiempo real. El mapa está bloqueado al sector de **Coro** y **Las Calderas**. Haz clic en cualquier estación para filtrar la bitácora.
            </p>

            <div className="public-map-layout">
              {/* Contenedor Leaflet */}
              <div className="public-map-container-wrapper" style={{ position: 'relative' }}>
                {/* Selector de capas flotante */}
                <div className="map-layer-selector" style={{ top: '10px', left: '10px' }}>
              ...
              <div className="public-map-sidebar">
                <h4 style={{ color: 'var(--text-main)', margin: '0 0 12px 0', fontSize: '0.95rem', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '8px' }}>
                  {paradaSeleccionada ? `📍 Arribos: ${paradaSeleccionada}` : '📍 Selecciona una Parada'}
                </h4>
                ...
                <div className="public-map-sidebar-list">
                  {!paradaSeleccionada ? (
                    <div style={{ textAlign: 'center', padding: '30px 10px', color: 'var(--text-muted)', fontSize: '0.8rem' }}>
                      ...
                      Presiona un marcador verde en el mapa para ver la bitácora de arribos de esa estación.
                    </div>
                  ) : llegadasFiltradas.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: '35px 10px', color: 'var(--text-muted)', fontSize: '0.8rem' }}>
                    ...
                    llegadasFiltradas.slice(0, 5).map((ll, idx) => (
                      ...
                      >
                        <span style={{ fontSize: '0.85rem', color: 'var(--text-main)', fontWeight: '500' }}>{ll.nombreUnidad}</span>
                        <span style={{ fontSize: '0.95rem', color: '#3b82f6', fontWeight: 'bold' }}>{ll.horaLlegada}</span>
                      </div>
                    ))
                  )}
                </div>
