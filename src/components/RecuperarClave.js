...
function RecuperarClave({ onVolver }) {
  ...
  const handleBuscarUsuario = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/recuperar-clave/${usuarioInput.trim()}`);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h2 style={{ textAlign: 'center', color: '#f1c40f', marginBottom: '5px', fontSize: '1.4rem' }}>
          Recuperar Acceso
        </h2>
        ...
        {paso === 1 && (
          <form onSubmit={handleBuscarUsuario}>
            ...
            <button type="submit" className="btn-login" style={{ background: 'linear-gradient(135deg,#f1c40f,#f39c12)', color: '#0f172a' }} disabled={loading}>
              {loading ? 'BUSCANDO...' : '🔍 BUSCAR USUARIO'}
            </button>
            <button type="button" onClick={onVolver} style={{ width: '100%', marginTop: '12px', background: 'transparent', border: '1px solid rgba(255,255,255,0.15)', color: 'var(--text-muted)', padding: '12px', borderRadius: '12px', cursor: 'pointer', fontSize: '0.9rem' }}>
              ← Volver al Login
            </button>
          </form>
        )}
        ...
        {paso === 2 && (
          <form onSubmit={handleCambiarClave}>
            <div style={{ background: 'rgba(241,196,15,0.08)', border: '1px solid rgba(241,196,15,0.3)', borderRadius: '12px', padding: '15px', marginBottom: '20px' }}>
              <p style={{ color: '#f1c40f', margin: 0, fontSize: '0.9rem', fontWeight: 600, marginBottom: '5px' }}>🔐 Pregunta de Seguridad:</p>
              <p style={{ color: 'var(--text-muted)', margin: 0, fontSize: '0.95rem' }}>{pregunta}</p>
            </div>
            <div className="input-group">
            ...
            <button type="submit" className="btn-login" style={{ background: 'linear-gradient(135deg,#3b82f6,#2563eb)' }} disabled={loading}>
              {loading ? 'VERIFICANDO...' : '🔑 CAMBIAR CONTRASEÑA'}
            </button>
            <button type="button" onClick={() => setPaso(1)} style={{ width: '100%', marginTop: '12px', background: 'transparent', border: '1px solid rgba(255,255,255,0.15)', color: 'var(--text-muted)', padding: '12px', borderRadius: '12px', cursor: 'pointer', fontSize: '0.9rem' }}>
        ...
        {paso === 3 && (
          <div style={{ textAlign: 'center' }}>
            ...
            <p style={{ color: '#3b82f6', fontSize: '1.1rem', fontWeight: 600, marginBottom: '10px' }}>
